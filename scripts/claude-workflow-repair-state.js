...
function findWorkflowRoot(startDir) {
  ...
  while (true) {
    if (exists(path.join(current, 'claude-workflow'))) {
      return current;
    }
...
function reconstruct(root, project) {
  const workflowDir = path.join(root, 'claude-workflow');
  ...
  if (phase4) {
    ...
    if (allPhase4TasksComplete(content)) {
      return route(root, project, 5, 'phase4-progress.md', true);
    }
...
function route(root, project, phase, phaseFileName, crossesBoundary, task = 'N/A') {
  const projectDir = path.join(root, 'claude-workflow', project);
  ...
  return {
    ...
    task,
    nextCommand: `/claude-workflow-phase${phase} ${project}`,
    phaseFile,
...
function stateLooksValid(root, project, content) {
  ...
  if (!new RegExp(`^/claude-workflow-phase${phase}\\s+${project.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`).test(nextCommand)) {
    return false;
  }
...
function stateContent(routeResult) {
  ...
  return [
    ...
    'cache_file: N/A',
    `last_command: node scripts/claude-workflow-repair-state.js ${routeResult.project}`,
...
function main() {
  ...
  if (!root) {
    process.stdout.write('Workflow state repair: skipped - no claude-workflow directory found\n');
    return;
  }
  ...
  if (exists(stateFile)) {
    ...
    if (stateLooksValid(root, selection.project, content)) {
      routeResult = reconstruct(root, selection.project);
  ...
  routeResult = routeResult || reconstruct(root, selection.project);
  if (routeResult.complete) {
    process.stdout.write(`Workflow state repair: skipped - ${routeResult.reason}\n`);
    return;
  }
...
try {
  ...
  process.stderr.write(`[claude-workflow state repair failed] ${error.message}\n`);
  process.exitCode = 1;
}
