...
function findWorkflowRoot(startDir) {
  ...
  while (true) {
    if (fs.existsSync(path.join(current, 'claude-workflow'))) {
      return current;
    }
...
function main() {
  ...
  const root = findWorkflowRoot(cwd);
  if (!root) return;

  const states = findStateFiles(path.join(root, 'claude-workflow'));
  ...
  const lines = [
    'Claude Workflow compact resume:',
...
try {
  ...
  process.stderr.write(`[claude-workflow compact hook skipped] ${error.message}\n`);
}
