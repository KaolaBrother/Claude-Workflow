...
const targetAgentsDir = path.join(targetCodexDir, 'agents', 'codex-workflow');
const targetConfig = path.join(targetCodexDir, 'config.toml');
const beginMarker = '# BEGIN codex-workflow agents';
const endMarker = '# END codex-workflow agents';
...
function main() {
  ...
  console.log(`Codex workflow agent profiles: copied ${copied.length} profiles`);
  console.log(`Codex workflow agent profiles: config ${configStatus} at ${path.relative(projectRoot, targetConfig)}`);
